...
InputState::InputState(int width, int height)
    ...
    canvases_.back().second->AddButton({0.55f, 0, 1, 0.2f}, T_MINUS_BUTTON, event); // decrement x
    event.increments = {-1, 0};
    canvases_.back().second->AddButton({0, 0.5f, 0.45f, 0.7f}, T_PLUS_BUTTON, event); // increment y
    event.increments = {0, -1};
